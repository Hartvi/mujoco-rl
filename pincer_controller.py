...
class PincerController:
    ...
    def __init__(
        ...
        self.max_translation_delta: float = self.MAX_LINEAR_SPEED * self.control_dt
        self.max_rotation_delta: float = self.MAX_ANGULAR_SPEED * self.control_dt
        self.max_distance_delta: float = self.MAX_DISTANCE_SPEED * self.control_dt
        self.body_id: int = self._name_id(mujoco.mjtObj.mjOBJ_BODY, self.OBJECT_BODY)
        self.object_joint_id: int = self._name_id(
            mujoco.mjtObj.mjOBJ_JOINT, "object_pose"
        )
        ...
        self.joint_id: int = self._name_id(
            mujoco.mjtObj.mjOBJ_JOINT, self.DISTANCE_JOINT
        )
        self.actuator_id: int = self._name_id(
        ...
        self.target_quat: np.ndarray[tuple[Any, ...], np.dtype[np.float64]] = data.qpos[
            self.object_qpos_id + 3 : self.object_qpos_id + 7
        ].copy()
    ...
    def reset(self) -> None:
        key_id: int = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_KEY, "touching")
        if key_id >= 0:
    ...
    def sync_target_to_pose(self) -> None:
        ...
        self.target_quat = self.data.qpos[
            self.object_qpos_id + 3 : self.object_qpos_id + 7
        ].copy()
    ...
    def observation(self) -> np.ndarray:
        ...
        orientation: np.ndarray[tuple[int], np.dtype[np.float64]] = np.zeros(
            3, dtype=np.float64
        )
    ...
    def apply_delta(self, action: np.ndarray) -> None:
        ...
        if action.size != 7:
            raise ValueError(
                f"Pincer action must contain 7 pose values, not {action.size}"
            )
        self.target_position += np.clip(
            action[:3], -self.max_translation_delta, self.max_translation_delta
        )
        ...
        self.target_distance = float(
            np.clip(
                ...
                + np.clip(action[6], -self.max_distance_delta, self.max_distance_delta),
                *self.distance_range,
            )
        )
        self.data.ctrl[self.actuator_id] = self.target_distance
    ...
    def _apply_pose_control(self) -> None:
        velocity: np.ndarray[tuple[Any, ...], np.dtype[np.float64]] = self.data.qvel[
            self.object_dof_id : self.object_dof_id + 6
        ]
        ...
        quat_error: np.ndarray[tuple[Any, ...], np.dtype[Any]] = self._quat_mul(
            self.target_quat,
            np.array([actual_quat[0], *(-actual_quat[1:])]),
        )
        if quat_error[0] < 0.0:
            quat_error *= -1.0
        rotation_error: np.ndarray[tuple[int], np.dtype[np.float64]] = np.zeros(
            3, dtype=np.float64
        )
        mujoco.mju_quat2Vel(rotation_error, quat_error, 1.0)
        torque = (
            self.ORIENTATION_KP * rotation_error - self.ORIENTATION_KD * velocity[3:]
        )
    ...
    def _rotvec_to_quat(rotvec: np.ndarray) -> np.ndarray:
        ...
        return np.concatenate(
            ([np.cos(angle / 2.0)], np.sin(angle / 2.0) * rotvec / angle)
        )
